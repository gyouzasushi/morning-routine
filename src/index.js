"use strict";
const addButton = document.getElementById("add_routine");
const nextButton = document.getElementById("add_complete");
const routineInput = document.getElementById("input_routine");
const routineField = document.getElementById("routine_field");
const ctx = document.createElement('canvas').getContext('2d');
const size = 16;
const routines = new Array();
const leftEdges = new Array();
const rightEdges = new Array();
const leftIndice = new Array();
const rightIndice = new Array();
const g = new Array();
let n = 0;
let leftCount = 0;
let rightCount = 0;
...
let inv = new Array();
let p = new Array();
function createrRoutineView(str, id, len, classname = 'none') {
...
function clearField() {
    while (routineField.firstChild)
        routineField.removeChild(routineField.firstChild);
}
...
function createArrowLeft(s, t, classname = 'none') {
...
function createArrowRight(s, t, classname = 'none') {
    const arrow = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    arrow.setAttribute('fill', 'none');
    arrow.setAttribute('stroke', 'black');
...
function addArrowLeft(s, t, classname = 'none') {
    leftEdges.push([s, t]);
    if (leftIndice[s] == 0) {
    ...
    for (const [s, t] of leftEdges)
        routineField.appendChild(createArrowLeft(s, t, classname));
    for (const [s, t] of rightEdges)
        routineField.appendChild(createArrowRight(s, t, classname));
}
function addArrowRight(s, t, classname = 'none') {
    rightEdges.push([s, t]);
    if (rightIndice[s] == 0)
        rightCount++, rightIndice[s] = rightCount;
    for (const [s, t] of leftEdges)
        routineField.appendChild(createArrowLeft(s, t, classname));
    for (const [s, t] of rightEdges)
        routineField.appendChild(createArrowRight(s, t, classname));
...
nextButton.onclick = () => {
    ...
    if (n < 2)
        return;
    queryMax = n * (n - 1) >> 1;
    while (leftIndice.length < n)
        leftIndice.push(0);
    while (rightIndice.length < n)
        rightIndice.push(0);
    while (g.length < n)
        g.push(new Array());
...
function sorting() {
    ...
    for (let s = 0; s < n; s++) {
        for (const t of g[s]) {
            clearField();
            if (inv[s] % 2 == 0)
                addArrowRight(inv[s], inv[t], 'arrow');
            if (inv[s] % 2 == 1)
                addArrowLeft(inv[s], inv[t], 'arrow');
        }
    }
    for (let i = 0; i < routines.length; i++) {
        document.getElementById('style').appendChild(document.createTextNode(getCssrule(i, inv[i] - i)));
